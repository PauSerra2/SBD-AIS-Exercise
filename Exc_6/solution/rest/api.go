...
func GetReceiptFile(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        w.Header().Set("Content-Disposition", "inline; filename="+order.GetFilename())
...
func PostOrder(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        ctx, cancel := context.WithTimeout(r.Context(), 5*time.Second)
        defer cancel()
        _, err = s3.PutObject(ctx, storage.OrdersBucket, dbOrder.GetFilename(), reader, int64(reader.Len()), minio.PutObjectOptions{
